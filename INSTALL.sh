...
then
{
  ...
  curl -o miniconda.sh https://repo.continuum.io/miniconda/Miniconda3-4.3.30.1-MacOSX-x86_64.sh
...
{  
  ...
  wget https://repo.continuum.io/miniconda/Miniconda3-4.3.30-Linux-x86.sh -O miniconda.sh
  chmod +x miniconda.sh
  ./miniconda.sh -b -p $ROOTDIR/thirdParty/miniconda -f
  # export PATH=$ROOTDIR/thirdParty/miniconda/bin:$PATH
  ln -sf $ROOTDIR/thirdParty/miniconda/bin/conda $ROOTDIR/bin/conda
