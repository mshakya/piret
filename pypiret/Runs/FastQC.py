...
class SGEPairedRunQC(SGEJobTask):
    ...
    def work(self):
        ...
        out_file = self.outdir + "/" + self.sample + ".stats.text"
    ...
    def requires(self):
        ...
        for samp, fastq in self.fastq_dic.iteritems():
            ...
            if isinstance(fastq, (list, tuple)):
                fqs = list(chain.from_iterable([fq.split(':') for fq in fastq]))
                print(fqs)
